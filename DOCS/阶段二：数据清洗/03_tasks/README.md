...
```text
DOCS/阶段二：数据清洗/03_tasks/task/active/runtime-g1/xxx.md
DOCS/阶段二：数据清洗/03_tasks/task/active/service-s1-g2/xxx.md
```
...
4. `DOCS/阶段二：数据清洗/执行记录/` 中与当前 L2、当前 L3 或直接上游接口相关的记录。
...
5. 将本次执行记录和当前状态快照写入 `DOCS/阶段二：数据清洗/执行记录/`。
6. 将任务文件从 `DOCS/阶段二：数据清洗/03_tasks/task/active/<功能组>/` 移到 `DOCS/阶段二：数据清洗/03_tasks/task/completed/<功能组>/`。
