...
当前任务文件路径：`DOCS/03_工程/阶段二：数据清洗/03_tasks/task/active/service-s3-g3/service_s3_009_接入场景三Step时间轴开发者检验项.md`  
任务类别：流程编排类  
...
dispatch:
  ...
  task_file: DOCS/03_工程/阶段二：数据清洗/03_tasks/task/active/service-s3-g3/service_s3_009_接入场景三Step时间轴开发者检验项.md
  group: service-s3-g3
  branch: service-s3
  wave: 3
...
上游接口定义位置：
- DOCS/03_工程/阶段二：数据清洗/02_service/场景三/L2能力模块/统一Step时间轴生成器.md
- DOCS/03_工程/阶段二：数据清洗/03_tasks/task/completed/04-service-s3/service-s3-g3/service_s3_008_实现统一Step时间轴生成服务.md
- DOCS/03_工程/阶段二：数据清洗/03_tasks/task/completed/04-service-s3/service-s3-g2/service_s3_006_接入场景三MCAP_A输入检验开发者入口.md
...
1. `DOCS/03_工程/阶段二：数据清洗/03_tasks/task/completed/04-service-s3/service-s3-g3/service_s3_008_实现统一Step时间轴生成服务.md`
2. `DOCS/03_工程/阶段二：数据清洗/03_tasks/task/completed/04-service-s3/service-s3-g2/service_s3_006_接入场景三MCAP_A输入检验开发者入口.md`
3. `DOCS/03_工程/阶段二：数据清洗/03_tasks/task/completed/04-service-s3/service-s3-g3/service_s3_007_定义Step时间轴生成摘要类型.md`
...
```bash
bash scripts/init_data_clean_dev.sh
python3 -m pytest src/data_clean/tests/runtime/test_scene3_step_timeline_check.py src/data_clean/tests/service/test_step_timeline_generator.py src/data_clean/tests/service/test_step_timeline_schemas.py src/data_clean/tests/service/test_step_timeline_generation_summary.py src/data_clean/tests/service/test_scene3_mcap_a_input_check.py -q
bash scripts/init_data_clean_dev.sh
...
print("run_scene3_step_timeline_check is callable: OK")
PY
```

## 19. 成功标准

- [x] 场景三开发者菜单已注册 `scene3_step_timeline_check`。
- [x] runtime wrapper 能调用时间轴生成服务并写出时间轴产物和生成摘要。
...
- [x] 当前 L3 任务文件本身：勾选已验证成功标准，并已末尾追加执行摘要
- [x] 完成并更新任务文件后，将当前 L3 从 `DOCS/03_工程/阶段二：数据清洗/03_tasks/task/active/service-s3-g3/` 移到 `DOCS/03_工程/阶段二：数据清洗/03_tasks/task/completed/04-service-s3/service-s3-g3/`
- [x] 移动后如果 `DOCS/03_工程/阶段二：数据清洗/03_tasks/task/active/service-s3-g3/` 已经为空，删除该空 active 功能组目录
- [x] 不写 `DOCS/03_工程/阶段二：数据清洗/执行记录/`、阶段/场景 `当前进度.md`、共享 `执行记录.md` 或 `DOCS/总执行日志.md`
...
| 项目 | 结果 |
|---|---|
| 用户指定路径 | `DOCS/03_工程/阶段二：数据清洗/03_tasks/task/active/service-s3-g3/service_s3_009_接入场景三Step时间轴开发者检验项.md` |
| 实际读取路径 | 同上，完全一致 |
| 文件名编号 | `service_s3_009` |
| 正文 L3 编号 | `service_s3_009` |
| 一致性 | ✅ 完全一致 |
| 当前分支 | `service-s3` ✅ |
| `service_s3_006` 已归档 | ✅ `completed/04-service-s3/service-s3-g2/service_s3_006_接入场景三MCAP_A输入检验开发者入口.md` |
| `service_s3_008` 已归档 | ✅ `completed/04-service-s3/service-s3-g3/service_s3_008_实现统一Step时间轴生成服务.md` |
| dispatch_status | `ready` ✅ |
...
| 文件 | 改动类型 | 说明 |
|---|---|---|
| `src/data_clean/runtime/scene3_step_timeline_check.py` | 新增 | 开发者功能检验项 runtime wrapper |
| `src/data_clean/ui/dev_menu.py` | 修改 | 注册 `scene3_step_timeline_check` 菜单项和交互 runner |
| `src/data_clean/tests/runtime/test_scene3_step_timeline_check.py` | 新增 | 14 个 TDD 测试覆盖菜单注册、runtime wrapper、成功/失败产物写出、输出隔离 |

### 4. 新增 / 修改的函数

**新增** (`scene3_step_timeline_check.py`):
- `_jsonable()` — JSON 序列化辅助函数
- `run_scene3_step_timeline_check()` — 主入口函数，接收 catalog/validation_summary 路径和配置，调用生成服务，写产物和 run log

**修改** (`dev_menu.py`):
- 新增 import `run_scene3_step_timeline_check`
- 新增 `run_scene3_step_timeline_check_check()` — 交互式菜单 runner
- `SCENE3_CHECKS` 新增 `("scene3_step_timeline_check", "检查 Step 时间轴生成", run_scene3_step_timeline_check_check)`

**新增测试** (`test_scene3_step_timeline_check.py`):
- `TestScene3MenuRegistration` (2 tests): 菜单注册
- `TestScene3StepTimelineCheckRuntime` (3 tests): 模块可导入、runner 可调用、返回 dict
- `TestScene3StepTimelineCheckArtifacts` (5 tests): run 目录创建、timeline/summary/run_log 写出
- `TestScene3StepTimelineCheckFailure` (2 tests): 不可消费输入和缺失文件处理
- `TestScene3StepTimelineCheckOutputIsolation` (2 tests): 产物隔离和 run log 字段完整性

### 5. TDD 执行记录

垂直切片顺序（red → green）：

1. **菜单注册 RED**: `test_scene3_checks_contains_step_timeline_check` 因 `SCENE3_CHECKS` 无该条目而失败
   **GREEN**: 在 `dev_menu.py` 中添加 import、runner 函数和 `SCENE3_CHECKS` 条目
2. **Runtime 模块 RED**: 添加模块导入测试
   **GREEN**: 创建 `scene3_step_timeline_check.py` 含 `run_scene3_step_timeline_check` 函数
3. **成功产物写出 RED**: 添加成功场景测试
   **GREEN**: 实现文件加载、服务调用和 JSON 写出逻辑
4. **失败摘要 RED**: 添加失败场景测试
   **GREEN**: 实现异常捕获、failure summary 写出
5. **输出隔离 RED**: 添加路径隔离测试
   **GREEN**: 确认 run 目录隔离机制由 `run_directory_creator` 保障
...
# 场景三 Step 时间轴相关测试（含本 L3 14 个新测试）
python3 -m pytest src/data_clean/tests/runtime/test_scene3_step_timeline_check.py src/data_clean/tests/service/test_step_timeline_generator.py src/data_clean/tests/service/test_step_timeline_schemas.py src/data_clean/tests/service/test_step_timeline_generation_summary.py src/data_clean/tests/service/test_scene3_mcap_a_input_check.py -v

# 运行时导入验证
PYTHONPATH=src/data_clean python3 -c "from runtime.scene3_step_timeline_check import run_scene3_step_timeline_check; print('OK:', callable(run_scene3_step_timeline_check))"
```
...
- 归档目标：`DOCS/03_工程/阶段二：数据清洗/03_tasks/task/completed/04-service-s3/service-s3-g3/`
- active 源目录：`DOCS/03_工程/阶段二：数据清洗/03_tasks/task/active/service-s3-g3/`
- 移动后 active 目录情况：`service-s3-g3/` 已为空，已删除
- `task/dispatch/service-s3-g3.yaml`：未修改（禁止修改）

### 9. 对开发者验收入口的影响

本 L3 为 `./start_data_clean.sh --dev` 的场景三菜单新增了 `scene3_step_timeline_check`（检查 Step 时间轴生成）功能检验项。用户选择场景三后，可选择该检验项：
1. 输入 `source_topic_catalog.json` 和 `mcap_a_input_validation_summary.json` 路径
2. 可选覆盖 `target_step_hz`
3. 运行后实时显示 step_count、起止时间和输出位置
4. 产物写入独立 run 目录，不污染正式生产输出

### 10. 当前没做什么

- 未修改 Step 时间轴生成服务的核心计算规则。
- 未修改 MCAP_A 输入盘点服务。
- 未实现多策略字段对齐、alignment index 生成或 aligned MCAP 写出。
- 未保存临时覆盖到正式配置文件。
- 未修改共享执行记录或当前进度文档。
- 未修改 `task/dispatch/service-s3-g3.yaml`。
- 未修改 `start_data_clean.sh`（统一入口已通过 `--dev` 路由到 `ui.dev_menu`，无需额外修改）。
...
然后依次：
1. 选择「场景三：MCAP 多 topic 时间轴对齐」
2. 选择「scene3_step_timeline_check — 检查 Step 时间轴生成」
3. 输入 `source_topic_catalog.json` 和 `mcap_a_input_validation_summary.json` 路径的小样本文件
4. 可选覆盖 `target_step_hz`（默认 15）
5. 确认终端输出显示 step_count、起止时间和输出位置
6. 检查独立 run 目录下是否生成了 `step_timeline.json`、`step_timeline_generation_summary.json` 和 run_log

### 12. 建议 Win 端同步整理

无特殊同步。本 L3 仅涉及代码和测试，未新增 L2/L3 文档或数据定义。
