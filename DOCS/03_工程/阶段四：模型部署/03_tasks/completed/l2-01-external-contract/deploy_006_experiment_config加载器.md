...
本任务属于阶段四（模型部署），L1 ACT 模型部署线，L2 `l2-01-external-contract`（外部参数加载与契约校验闭环），L3 编号 `deploy_006`，改造类型 `source-adaptation`，验收模式 `direct-local`，真机风险 `none`，开发分支 `feat/model_deploy/l2-01-external-contract`，集成分支 `model_deploy`。
