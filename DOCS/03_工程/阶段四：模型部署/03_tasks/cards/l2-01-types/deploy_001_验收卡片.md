# L3 验收卡片：deploy_001 重构 action_spec 为 TCP+width 结构
...
|---|---|
| L3 编号 | `deploy_001` |
| L3 文件 | `DOCS/03_工程/阶段四：模型部署/03_tasks/task/active/l2-01-types/deploy_001_重构action_spec为TCP_width结构.md` |
| 所属 L2 | `l2-01-types` |
...
确认执行 agent 是否完成 `重构 action_spec 为 TCP+width 结构` 的唯一目标，并在 Ubuntu 22.04 无外联硬件条件下给出可验证结论。

## 验收模式说明

主模式：`direct-local`。辅助模式：`[]`。

验收重点：S1/S2 维度、段序与 round-trip。
...
```text
DOCS/03_工程/阶段四：模型部署/05_acceptance/l2-01-types/logs/deploy_001_acceptance_round_<n>.md
```
