...
```text
用户指定路径：DOCS/阶段二：数据清洗/03_tasks/task/active/service-s2-g4/service_s2_015_实现逐cell中值EMA触觉滤波与审计.md
实际读取路径：DOCS/阶段二：数据清洗/03_tasks/task/active/service-s2-g4/service_s2_015_实现逐cell中值EMA触觉滤波与审计.md
...
- 上游依赖：已读取归档任务 `DOCS/阶段二：数据清洗/03_tasks/task/completed/03-service-s2/service-s2-g4/service_s2_013_定义触觉滤波器配置输入审计记录和结果类型.md` 和 `DOCS/阶段二：数据清洗/03_tasks/task/completed/03-service-s2/service-s2-g4/service_s2_014_实现触觉滤波片段切分与接触变化边界.md`，确认可复用 `TactileFilterConfig`、`TactileFilterSampleRecord`、`TactileFilterResult` 和 `split_tactile_segments`。
