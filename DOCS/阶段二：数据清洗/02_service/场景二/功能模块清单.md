...
| 2 | 数据补全器 | 数据计算类 | 只消费异常检测器输出，对可修复的已有样本执行插值、SLERP 或邻近补全，并记录不可补全问题。 | P0 |
| 3 | 位姿滤波器 | 数据计算类 | 对已完成明显坏点处理的位姿序列进行平滑，降低高频噪声且保留真实运动趋势。 | P0 |
| 4 | 触觉滤波器 | 数据计算类 | 对已完成明显坏点处理的触觉序列进行平滑，降低噪声且保留接触变化。 | P0 |
| 5 | MCAP_A 生成器 | 数据读写类 | 将补全和滤波后的位姿、触觉、夹爪等主数据写入 MCAP_A，作为场景二 validated 主 MCAP。 | P0 |
| 6 | IK 求解与 MCAP_B 生成器 | 数据计算类 / 数据读写类 | 从 MCAP_A 读取位姿并求解关节角，生成只用于机器人验证诊断的 MCAP_B。 | P1 |
| 7 | 关节限制检查器 | 数据计算类 | 基于 MCAP_B 检查 IK 无解、关节角、速度、加速度和 workspace 问题，并输出问题时间段。 | P1 |
...
- 模块类别：数据计算类。
- 输入：cleaned MCAP 中的原始 TCP / common frame 位姿序列、触觉序列、夹爪宽度序列，以及检测规则和阈值。
- 输出：样本级异常记录、无样本缺失区间、可选聚合摘要、异常类型、点级证据、异常原因摘要和建议处理方式；不直接修改序列。
- 上游依赖：场景一 `CleanedMcap`；优先消费 `CommonFrameTcpPose`、触觉 topic 和 gripper width topic。
...
- 一句话目标：只消费异常检测器输出，对可修复的已有样本进行插值、SLERP、邻近保持或其他可追溯补全。
- 模块类别：数据计算类。
...
- 一句话目标：将补全和滤波后的位姿、触觉、夹爪等主数据写入 MCAP_A，作为场景二 validated 主 MCAP。
- 模块类别：数据读写类。
- 输入：cleaned MCAP 原文件、补全后序列、滤波后位姿序列、滤波后触觉序列、夹爪序列和不可补全时间段。
...
- 一句话目标：从 MCAP_A 读取 TCP / common frame 位姿并求解关节角，生成只用于机器人验证诊断的 MCAP_B。
- 模块类别：数据计算类 / 数据读写类。
- 输入：MCAP_A 中的 TCP / common frame 位姿序列、不可补全时间段、机器人运动学参数和 IK 求解配置。
- 输出：MCAP_B、IK 成功/失败时间段、求解摘要和 IK 元数据。
...
- 后续可拆 L3 方向：定义 MCAP_B 数据概念；实现 IK 求解和 MCAP_B 写出；补充 IK 失败标注验收。
