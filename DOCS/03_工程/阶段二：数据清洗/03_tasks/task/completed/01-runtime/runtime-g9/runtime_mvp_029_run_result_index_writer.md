...
- DOCS/03_工程/阶段二：数据清洗/03_tasks/task/active/runtime-g9/runtime_mvp_027_processing_manifest_writer.md
...
2. `DOCS/03_工程/阶段二：数据清洗/01_runtime_mvp/L2数据定义/RunResultIndex.md`
3. `DOCS/03_工程/阶段二：数据清洗/01_runtime_mvp/L2数据定义/ProcessingManifest.md`
4. `DOCS/03_工程/阶段二：数据清洗/01_runtime_mvp/L2数据定义/ErrorSummary.md`
5. `DOCS/03_工程/阶段二：数据清洗/01_runtime_mvp/L2数据定义/PipelineResult.md`
6. `DOCS/03_工程/阶段二：数据清洗/01_runtime_mvp/L2数据定义/RunDirectoryLayout.md`
7. `DOCS/03_工程/阶段二：数据清洗/01_runtime_mvp/L2数据定义/RunArtifactPath.md`

### 必读相关微元任务记录

1. `DOCS/03_工程/阶段二：数据清洗/03_tasks/task/active/runtime-g2/runtime_mvp_004_run_directory_types.md`
2. `DOCS/03_工程/阶段二：数据清洗/03_tasks/task/active/runtime-g9/runtime_mvp_026_manifest_error_types.md`
3. `DOCS/03_工程/阶段二：数据清洗/03_tasks/task/active/runtime-g9/runtime_mvp_027_processing_manifest_writer.md`
4. `DOCS/03_工程/阶段二：数据清洗/03_tasks/task/active/runtime-g9/runtime_mvp_028_error_summary_writer.md`
...
```bash
python3 -m pytest src/data_clean/tests/runtime/test_run_result_index_writer.py -q
```

## 15. 成功标准

- [x] 能在临时 run 目录内写出 `run_result.json`。
- [x] 成功路径 `run_result.json` 指向 `processing_manifest.json`，且不要求 `error_summary_path`。
- [x] 失败路径 `run_result.json` 指向 `error_summary.json`，且不要求 `manifest_path`。
- [x] `run_result.json` 可被 JSON 解析，并包含 `schema_version`、`run_id`、`status`、`run_dir`、`run_log_path`、`scene_results`、`created_at`。
- [x] 路径逃逸、成功缺 manifest、失败缺 error summary 时失败清楚。
...
- **新增**: `src/data_clean/runtime/run_result_index_writer.py` - run_result.json 写入器实现
- **修改**: `src/data_clean/schemas/manifest_types.py` - 添加 RunResultIndex.to_dict() 方法，修改 scene_results 类型为 list[dict]
- **新增**: `src/data_clean/tests/runtime/test_run_result_index_writer.py` - 8个测试用例

### 2. 新增或修改了哪些函数 / 测试

- `write_run_result_index()` - 主写入函数，接受 run_id, status, run_dir, run_log_path, scene_results, manifest_path, error_summary_path, layout
- `RunResultIndexError` - 自定义异常类
- `_validate_path_inside_run_dir()` - 路径逃逸检查
- `_scene_result_to_dict()` - SceneResult 序列化

### 3. 如何运行验收

```bash
cd /home/hit/ROS/src/data_clean
python3 -m pytest tests/runtime/test_run_result_index_writer.py -v
```

### 4. TDD red / green / refactor 执行

- Red: 运行测试 -> 8 failures (导入问题)
- Green: 修复模块导入，添加 to_dict() 方法 -> 8 passed
- Refactor: 代码结构优化保持不变
...
- 没有定义新的 Types（消费现有 RunResultIndex）
- 没有写其他运行记录文件
- 没有接入 Runtime 入口或调度器
- 没有创建真实 run 目录
...
- 确认 RunResultIndex 在真实 Runtime 入口中的调用位置
- 确认 manifest/error_summary 写入后调用 run_result_index 的时序
