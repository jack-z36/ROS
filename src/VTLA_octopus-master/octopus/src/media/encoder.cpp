...
int Encoder::open(const std::string& filename, std::map<std::string, std::string> options)
{
    if (!audio_enabled_ && !video_enabled_) {
        LOG_E("[    ENCODER] audio and video streams are all disabled.");
        return av::INVALID;
    }

    LOG_I("[   ENCODER] {}, options = {}", filename, options);
    ...
    if (!(fmt_ctx_->oformat->flags & AVFMT_NOFILE)) {
        if (avio_open(&fmt_ctx_->pb, filename.c_str(), AVIO_FLAG_WRITE) < 0) {
            LOG_E("[   ENCODER] can not open the output file: {}", filename);
            return -1;
        }
    }

    if (avformat_write_header(fmt_ctx_, nullptr) < 0) {
        LOG_E("[   ENCODER] can not write the header to the output file: ", filename);
        return -1;
    }

    av_dump_format(fmt_ctx_, 0, filename.c_str(), 1);

    LOG_I("[   ENCODER] [{}] is opened", filename);
...
{
    LOG_I("[   ENCODER] [V] <<< [{}], {}", codec_name, av::to_string(vfmt));

    const auto stream = avformat_new_stream(fmt_ctx_, nullptr);
    if (!stream) {
        LOG_E("[   ENCODER] failed to create stream");
        return -1;
    }
    ...
    if (!vcodec_ctx_) {
        LOG_E("[   ENCODER] failed to alloc the video encoder context.");
        return -1;
    }
    ...
    if (vfmt.hwaccel != AV_HWDEVICE_TYPE_NONE) {
        if (av::hwaccel::setup_for_encoding(vcodec_ctx_, vfmt.hwaccel) != 0) {
            LOG_E("[   ENCODER] failed to set hardware device for encoding.");
            return -1;
        }
    }

    if (avcodec_open2(vcodec_ctx_, video_encoder, &options) < 0) {
        LOG_E("[   ENCODER] filed to open the video encoder: {}", codec_name);
        return -1;
    }
...
{
    LOG_I("[   ENCODER] [A] <<< [{}], {}", codec_name, av::to_string(afmt));

    const auto stream = avformat_new_stream(fmt_ctx_, nullptr);
    if (!stream) {
        LOG_E("[   ENCODER] filed to create audio streams.");
        return -1;
    }
    ...
    if (!acodec_ctx_) {
        LOG_E("[   ENCODER] failed to alloc the audio encoder context.");
        return -1;
    }
    ...
    if (avcodec_open2(acodec_ctx_, audio_encoder, &options) < 0) {
        LOG_E("[   ENCODER] failed to open audio encoder");
        return -1;
    }
    ...
    if (astream_idx_ >= 0) {
        LOG_I("[   ENCODER] [A] >>> [{}], sample_rate={}:sample_fmt={}:channels={}:tbc={}:tbn={}",
...
{
    if (!ready_ || running_) {
        LOG_W("[   ENCODER] already running or not ready");
        return -1;
    }
    ...
    thread_  = std::jthread([this]() {
        ...
        LOG_I("[    ENCODER] encoded frames: {}, exited", vcodec_ctx_->frame_num);
    });
...
std::pair<int, int> Encoder::video_sync_process(av::frame& vframe)
{
    ...
    if (num_frames == 0) {
        LOG_W("[V] drop the frame.");
    }
    else if (num_frames > 1) {
        LOG_W("[V] duplicated {} frames and {} previous frames.", num_frames - 1, num_pre_frames);
    }
...
int Encoder::process_video_frames()
{
    ...
    for (auto i = 0; i < num_frames; ++i) {
        ...
        if (encoding_frame) {
            ...
            if (vsync_ == av::vsync_t::cfr) encoding_frame->pts = expected_pts_;
        }
        ...
        while (ret >= 0) {
            ...
            if (v_last_dts_ != AV_NOPTS_VALUE && v_last_dts_ >= packet_->dts) {
                LOG_W("[V] drop the packet with dts {} <= {}", packet_->dts, v_last_dts_);
                continue;
            }
            v_last_dts_ = packet_->dts;

            LOG_D("[V] pts = {:>14d}, dts = {:>14d}, ts = {:.3%T}", packet_->pts, packet_->dts,
            ...
            if (av_interleaved_write_frame(fmt_ctx_, packet_.get()) != 0) {
                LOG_E("[V] failed to write the the packet to file.");
                return -1;
            }
        }
...
int Encoder::process_audio_frames()
{
    ...
    while (!(eof_ & A_ENCODING_EOF) && (abuffer_->size() >= acodec_ctx_->frame_size || asrc_eof_)) {
        ...
        if ((abuffer_->size() >= acodec_ctx_->frame_size) || (!abuffer_->empty() && asrc_eof_)) {
            ...
            aframe->ch_layout   = stream->codecpar->ch_layout;
        ...
        while (ret >= 0) {
            ...
            if (a_last_dts_ != AV_NOPTS_VALUE && a_last_dts_ >= packet_->dts) {
                LOG_W("[A] drop the frame: dts {} <= {}", packet_->dts, a_last_dts_);
                continue;
            }
            a_last_dts_ = packet_->dts;

            LOG_I("[A] pts = {:>14d}, dts = {:>14d}, ts = {:.3%T}", packet_->pts, packet_->dts,
                  av::clock::ns(packet_->pts, stream->time_base));

            packet_->stream_index = astream_idx_;

            if (av_interleaved_write_frame(fmt_ctx_, packet_.get()) != 0) {
                LOG_E("[A] failed to write the packet to the file.");
                return -1;
            }
        }
    }
...
void Encoder::close_output_file()
{
    ...
    if (av_write_trailer(fmt_ctx_) < 0) {
        LOG_E("[   ENCODER] failed to write trailer");
    }

    if (!(fmt_ctx_->oformat->flags & AVFMT_NOFILE) && avio_close(fmt_ctx_->pb) < 0) {
        LOG_E("[   ENCODER] failed to close the output file.");
    }
