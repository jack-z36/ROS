# L3 验收卡片：deploy_023 real-robot smoke test（safe-run + 急停）
...
|---|---|
| L3 编号 | `deploy_023` |
| L3 文件 | `DOCS/03_工程/阶段四：模型部署/03_tasks/task/active/l2-05-hardware/deploy_023_real_robot_smoke_test.md` |
| 所属 L2 | `l2-05-hardware` |
| 验收卡片 | `DOCS/03_工程/阶段四：模型部署/03_tasks/cards/l2-05-hardware/deploy_023_验收卡片.md` |
| 对应 L2 运行场景 | `[S5]` |
...
确认执行 agent 是否完成 `real-robot smoke test（safe-run + 急停）` 的唯一目标，并在 Ubuntu 22.04 无外联硬件条件下给出可验证结论。

## 验收模式说明

主模式：`hardware-blocked`。辅助模式：`[]`。

验收重点：S5 real-robot smoke test 无硬件必须 blocked。

## 本地验收命令

本 L3 的核心真实验收依赖外联硬件。Ubuntu 22.04 无硬件环境下不得执行真机命令；只允许做文档、风险、静态代码和 blocked 条件检查。
...
```text
DOCS/03_工程/阶段四：模型部署/05_acceptance/l2-05-hardware/logs/deploy_023_acceptance_round_<n>.md
```
