# L3 验收卡片：deploy_018 bridge 接入 IK 预检 + workspace 检查
...
|---|---|
| L3 编号 | `deploy_018` |
| L3 文件 | `DOCS/03_工程/阶段四：模型部署/03_tasks/task/active/l2-05-hardware/deploy_018_bridge_ik预检.md` |
| 所属 L2 | `l2-05-hardware` |
| 验收卡片 | `DOCS/03_工程/阶段四：模型部署/03_tasks/cards/l2-05-hardware/deploy_018_验收卡片.md` |
| 对应 L2 运行场景 | `[S1, S3]` |
| 验收模式 | `static-review` |
...
确认执行 agent 是否完成 `bridge 接入 IK 预检 + workspace 检查` 的唯一目标，并在 Ubuntu 22.04 无外联硬件条件下给出可验证结论。

## 验收模式说明

主模式：`static-review`。辅助模式：`[hardware-blocked]`。

验收重点：S1/S3 IK/workspace 代码边界可评审，SDK 实测 blocked。
...
```text
DOCS/03_工程/阶段四：模型部署/05_acceptance/l2-05-hardware/logs/deploy_018_acceptance_round_<n>.md
```
