...
本文件作为 `/home/hit/ROS` 工作区的文档目录索引使用。具体任务文档必须保存在 `/home/hit/ROS/DOCS` 目录下对应场景的位置中；后续某个场景产生的新文档，也必须继续加入该场景对应目录。

每次执行任务前，都必须要阅读 /home/hit/ROS/DOCS/public_rules.md 文档

当一次任务无法明确归入某个场景，或属于跨场景的零散维护、仓库管理、公共文档体系整理时，执行记录必须追加到 `/home/hit/ROS/DOCS/总执行日志.md`。

当我提到现在处于场景xx时，你都必须根据这里的索引，去加载这个场景下的5个文档，加载完了之后，你才能去执行任务。
比如我通过各种信息透露出现在在执行场景3时，你就必须按照目录去加载场景3的五个文档，然后再开始去执行任务

## 预读规则

当我问到跨场景零散任务、公共文档体系维护、仓库管理或无法判断属于哪个场景的问题时，你都必须先阅读：

- `/home/hit/ROS/DOCS/总执行日志.md`

当我问到与 Git 状态、提交、推送、拉取、远端、账号、分支或仓库同步相关的问题时，你都必须先阅读：

- `/home/hit/ROS/DOCS/git操作约束.md`

当我问到与 `/home/hit/ROS/src/baton_mini_sdk_demo` 相关的问题时，你都必须要先阅读：

- `/home/hit/ROS/src/baton_mini_sdk_demo/baton_mini_architecture.md`
- `/home/hit/ROS/src/baton_mini_sdk_demo/batonmini运行指南.md`

当我问到与 `/home/hit/ROS/src/gopro_camera_launch` 相关的问题时，你都必须要先阅读：
