...
- [ ] 缺少任一必需上游 result 时失败且不生成 MCAP_A。
- [ ] 合法输入可写出 MCAP_A 和 sidecar summary。
- [ ] MCAP_A 保留 cleaned MCAP 的 topic、消息类型、时间戳排序和样本数。
- [ ] pose/tactile/gripper 目标 topic 使用上游处理后序列，其他 topic 原样复制。
