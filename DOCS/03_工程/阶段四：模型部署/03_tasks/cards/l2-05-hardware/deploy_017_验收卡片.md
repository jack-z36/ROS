# L3 验收卡片：deploy_017 command_bridge_sender_node 骨架
...
|---|---|
| L3 编号 | `deploy_017` |
| L3 文件 | `DOCS/03_工程/阶段四：模型部署/03_tasks/task/active/l2-05-hardware/deploy_017_command_bridge骨架.md` |
| 所属 L2 | `l2-05-hardware` |
| 验收卡片 | `DOCS/03_工程/阶段四：模型部署/03_tasks/cards/l2-05-hardware/deploy_017_验收卡片.md` |
| 对应 L2 运行场景 | `[S1, S2]` |
| 验收模式 | `direct-local` |
| 辅助模式 | `[]` |
| 本地验收是否必须 | `true` |
...
确认执行 agent 是否完成 `command_bridge_sender_node 骨架` 的唯一目标，并在 Ubuntu 22.04 无外联硬件条件下给出可验证结论。

## 验收模式说明

主模式：`direct-local`。辅助模式：`[]`。

验收重点：S1/S2 bridge 骨架 import / AST / mock topic 检查。
...
```text
DOCS/03_工程/阶段四：模型部署/05_acceptance/l2-05-hardware/logs/deploy_017_acceptance_round_<n>.md
```
