...
```text
用户指定路径：DOCS/阶段二：数据清洗/03_tasks/task/active/service-s2-g3/service_s2_010_实现可靠片段切分与时间窗口换算.md
实际读取路径：DOCS/阶段二：数据清洗/03_tasks/task/active/service-s2-g3/service_s2_010_实现可靠片段切分与时间窗口换算.md
...
- 上游依赖：已读取归档任务 `DOCS/阶段二：数据清洗/03_tasks/task/completed/03-service-s2/service-s2-g2/service_s2_007_实现三模态补全计算规则.md` 和 `DOCS/阶段二：数据清洗/03_tasks/task/completed/03-service-s2/service-s2-g3/service_s2_009_定义位姿滤波器配置输入审计记录和结果类型.md`，确认本 L3 消费补全后的 pose 序列语义、缺失区间和 pose `unrepaired` / `skipped` 边界。
