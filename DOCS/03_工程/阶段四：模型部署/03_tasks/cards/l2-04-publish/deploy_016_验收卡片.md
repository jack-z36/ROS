# L3 验收卡片：deploy_016 L2-04 单测与 shadow-run 验证
...
|---|---|
| L3 编号 | `deploy_016` |
| L3 文件 | `DOCS/03_工程/阶段四：模型部署/03_tasks/task/active/l2-04-publish/deploy_016_单测与shadow_run验证.md` |
| 所属 L2 | `l2-04-publish` |
| 验收卡片 | `DOCS/03_工程/阶段四：模型部署/03_tasks/cards/l2-04-publish/deploy_016_验收卡片.md` |
...
确认执行 agent 是否完成 `L2-04 单测与 shadow-run 验证` 的唯一目标，并在 Ubuntu 22.04 无外联硬件条件下给出可验证结论。

## 验收模式说明

主模式：`direct-local`。辅助模式：`[env-blocked]`。

验收重点：S1/S2/S3/S4 单测与 shadow-run；缺 ROS 时 BLOCKED_ENV。
...
```text
DOCS/03_工程/阶段四：模型部署/05_acceptance/l2-04-publish/logs/deploy_016_acceptance_round_<n>.md
```
