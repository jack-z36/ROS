...
namespace av
{
    ...
    std::vector<TextureDescription> get_texture_desc(AVPixelFormat fmt)
    {
        ...
        default: LOG_E("unsupported pixel format: {}", av::to_string(fmt)); return {};
        }
    }
    ...
    QString get_shader_name(const av::vformat_t& fmt, bool hdr)
    {
        ...
        default:                 LOG_E("unsupported pixel format: {}", to_string(fmt.pix_fmt)); return {};
        }
    }

    QString get_frag_shader_path(const av::vformat_t& fmt, bool hdr)
    {
        return ":/resources/shaders/" + av::get_shader_name(fmt, hdr) + ".frag.qsb";
    }
} // namespace av
