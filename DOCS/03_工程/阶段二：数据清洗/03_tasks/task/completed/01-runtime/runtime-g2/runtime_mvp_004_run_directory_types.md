...
任务类别：数据定义类  
来源 L2 文件：`DOCS/03_工程/阶段二：数据清洗/01_runtime_mvp/L2能力模块/02_Run目录管理模块.md`
...
上游接口定义位置：DOCS/03_工程/阶段二：数据清洗/01_runtime_mvp/L2能力模块/01_Runtime运行上下文定义.md
...
1. `DOCS/03_工程/阶段二：数据清洗/01_runtime_mvp/L2能力模块/02_Run目录管理模块.md`
2. `DOCS/03_工程/阶段二：数据清洗/01_runtime_mvp/L2数据定义/RunDirectory.md`
3. `DOCS/03_工程/阶段二：数据清洗/01_runtime_mvp/L2数据定义/RunDirectoryLayout.md`
4. `DOCS/03_工程/阶段二：数据清洗/01_runtime_mvp/L2数据定义/RunArtifactPath.md`
5. `DOCS/03_工程/阶段二：数据清洗/01_runtime_mvp/L2数据定义/RunContext.md`
...
- [x] Run 目录相关 Types 能被正常 import。
- [x] 单场景 run_id 可生成 `YYYY-MM-DD_s1` 这类格式。
- [x] 全流程 run_id 可生成 `YYYY-MM-DD_all`。
- [x] 重复序号规则在测试中被覆盖。
- [x] 未创建真实 run 目录。
...
- L2 能力模块：`02_Run目录管理模块.md`
- L2 数据定义：`RunDirectory.md`、`RunDirectoryLayout.md`、`RunArtifactPath.md`、`RunContext.md`
- 约束文档：`L3编码执行原则.md`、`文件存放规范.md`、`01_runtime_mvp/执行约束.md`
- 代码文件：`data_clean_architecture.md`、`schemas/__init__.py`、`schemas/ros2_schemas.py`、`schemas/runtime_context.py`、`schemas/runtime_enums.py`
- 现有测试：`tests/runtime/test_run_directory_types.py`（已存在，等待实现）

### 修改文件

| 文件 | 操作 | 说明 |
| --- | --- | --- |
| `src/data_clean/schemas/run_directory_types.py` | 新增 | Run 目录 Types：RunArtifactKind、RunArtifactPath、RunDirectoryLayout、RunDirectory 与 run_id 命名辅助函数 |
| `src/data_clean/schemas/__init__.py` | 修改 | 导出新增 Types 和函数 |
| `src/data_clean/data_clean_architecture.md` | 修改 | 目录结构表新增 `run_directory_types.py` 行 |

### 新增函数 / 类型

| 名称 | 类型 | 说明 |
| --- | --- | --- |
| `RunArtifactKind` | Enum | `file` / `directory` 区分 |
| `RunArtifactPath` | dataclass | 运行记录文件或子目录的结构化路径引用 |
| `RunDirectoryLayout` | dataclass | run 目录内固定子路径集合 |
| `RunDirectory` | dataclass | 独立运行记录目录 |
| `build_base_run_id()` | function | 生成 base run_id（单场景 `YYYY-MM-DD_sN`，全流程 `YYYY-MM-DD_all`） |
| `ensure_unique_run_id()` | function | 冲突时追加 `_002`、`_003` 等短序号 |
| `build_run_id()` | function | 组合 base + unique 的完整 run_id 生成 |
| `build_run_directory()` | function | 构造 RunDirectory 描述符（不创建真实目录） |

### TDD 执行

测试文件 `test_run_directory_types.py` 已存在（8 个用例），覆盖：
- 单场景 run_id 格式
- 全流程 run_id 格式
- 重复序号递增
- `build_run_id` 冲突规则
- `build_run_directory` 不创建真实目录
- `RunDirectoryLayout` 声明所有 artifact 路径
- `RunArtifactPath` 拒绝非法 kind
- `RunDirectory` 拒绝路径逃逸
...
```bash
python3 -m pytest src/data_clean/tests/runtime/test_run_directory_types.py -v
```

结果：8 passed in 0.02s

### 当前没做什么

- 不创建真实 run 目录
- 不写 `run_log.json`、`config_snapshot.yaml`、`processing_manifest.json`、`error_summary.json`、`run_result.json`
- 不接入 Runtime 初始化流程
- 不修改 `start_data_clean.sh`

### 下一步建议

- `runtime_mvp_005`：实现 Run 目录创建器（实际创建目录和 `outputs/` 子目录）
- `runtime_mvp_006`：接入 RunContext 回填 `run_dir`
