...
> - 目标路径：`DOCS/03_工程/阶段四：模型部署/02_implement/agent_context/01_L1_ACT部署程序任务文档.md`。
...
> - 关联产物：
>   - Agent 索引：`00_INDEX.md`。
>   - 模块边界：`02_L1_ACT功能模块边界.md`（每个 L2 的输入/输出/负责/不负责/完成判据）。
>   - 模块协作：`03_L1_ACT功能模块协作架构.md`（模块间数据流、调用关系、对象所有权）。
...
| Agent | `03_L1_ACT功能模块协作架构.md` | 完整理解模块间协作关系。 | 高。协作关系契约。 |
...
每个 L2 的功能定义、输入、输出、负责/不负责内容、完成判据和代码层落点见 `02_L1_ACT功能模块边界.md`。
...
模块之间的协作接口、RAM 对象所有权、同步/异步边界详见 `03_L1_ACT功能模块协作架构.md`。
