...
def set_attention_implementation(config, implementation: str) -> None:
    """Set HF attention implementation flags on configs that expose them."""
    if config is None:
        return
    for name in ("_attn_implementation", "attn_implementation"):
        try:
            setattr(config, name, implementation)
        except Exception:
            pass
    text_config = getattr(config, "text_config", None)
    if text_config is not None and text_config is not config:
        set_attention_implementation(text_config, implementation)
...
class PaliGemmaWithExpertModel(
    ...
    def __init__(
        ...
        set_attention_implementation(vlm_config_hf, attention_implementation)
        ...
        set_attention_implementation(action_expert_config_hf, attention_implementation)
