...
```text
用户指定路径：DOCS/阶段二：数据清洗/03_tasks/task/active/service-s2-g5/service_s2_019_补充MCAP_A契约与场景三兼容验收.md
实际读取路径：DOCS/阶段二：数据清洗/03_tasks/task/active/service-s2-g5/service_s2_019_补充MCAP_A契约与场景三兼容验收.md
...
- 上游依赖：已读取归档任务 `DOCS/阶段二：数据清洗/03_tasks/task/completed/03-service-s2/service-s2-g5/service_s2_018_实现MCAP_A复制替换写出器.md`，确认 `MCAP_A_Writer`、写出计划、输出 contract 和 sidecar summary 可用于本 L3 契约测试。
