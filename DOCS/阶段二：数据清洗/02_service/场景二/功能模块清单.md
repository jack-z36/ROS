...
- `DOCS/阶段二：数据清洗/02_service/场景一/L2数据定义/ArmBaseTcpPose.md`
...
- 场景二优先消费 `ArmBaseTcpPose`（per RM65 base，frame_id 对应各臂 base link）。
...
| 3   | 位姿滤波器              | 数据计算类         | 对已完成明显坏点处理的 arm-base TCP pose 序列进行平滑，降低高频噪声且保留真实运动趋势。        | P0  |
| 4   | 触觉滤波器              | 数据计算类         | 对已完成明显坏点处理的触觉序列进行平滑，降低噪声且保留接触变化。                         | P0  |
| 5   | MCAP_A 生成器         | 数据读写类         | 将补全和滤波后的 arm-base TCP pose、触觉、夹爪等主数据写入 MCAP_A（pose topic = `arm_base_tcp_pose`，frame_id per RM65 base），作为场景二 validated 主 MCAP。 | P0  |
...
- 模块类别：数据计算类。
- 输入：数据补全器输出的 arm-base TCP pose 序列、不可补全时间段、pose 相关未修复 / 跳过样本和位姿滤波配置。
...
- 一句话目标：将补全和滤波后的 arm-base TCP pose、触觉、夹爪等主数据写入 MCAP_A（pose topic = `arm_base_tcp_pose`，frame_id per RM65 base），作为场景二 validated 主 MCAP。
- 模块类别：数据读写类。
- 输入：cleaned MCAP 原文件、补全后序列、滤波后 arm-base TCP pose 序列、滤波后触觉序列、夹爪序列和不可补全时间段。
