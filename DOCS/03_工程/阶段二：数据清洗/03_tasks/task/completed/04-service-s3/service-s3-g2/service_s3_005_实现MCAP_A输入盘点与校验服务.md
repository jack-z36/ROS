...
当前任务文件路径：`DOCS/03_工程/阶段二：数据清洗/03_tasks/task/active/service-s3-g2/service_s3_005_实现MCAP_A输入盘点与校验服务.md`  
任务类别：数据计算类  
...
dispatch:
  ...
  task_file: DOCS/03_工程/阶段二：数据清洗/03_tasks/task/active/service-s3-g2/service_s3_005_实现MCAP_A输入盘点与校验服务.md
  group: service-s3-g2
  branch: service-s3
  wave: 2
...
1. `DOCS/03_工程/阶段二：数据清洗/03_tasks/task/completed/04-service-s3/service-s3-g2/service_s3_004_定义MCAP_A输入盘点与校验类型.md`
2. `DOCS/03_工程/阶段二：数据清洗/03_tasks/task/completed/04-service-s3/service-s3-g1/service_s3_001_定义场景三配置与schema.md`
3. `DOCS/03_工程/阶段二：数据清洗/03_tasks/task/completed/03-service-s2/service-s2-g5/service_s2_019_补充MCAP_A契约与场景三兼容验收.md`
...
- [x] 合法 MCAP_A + completed summary 可输出 `SourceTopicCatalog` 和 `McapAInputValidationSummary(status=consumable)`。
- [x] MCAP_A 缺失、summary 缺失 / 不可读 / 不一致会输出 hard fail。
- [x] 左右图像基准 topic 缺失、无共同有效时间范围会阻塞。
- [x] 非基准字段问题只进入 warning 或 unavailable，不阻塞主链路。
- [x] 未映射 topic 被记录为只读盘点，不作为错误。
- [x] 已说明本 L3 与 `./start_data_clean.sh --dev` 开发者验收入口的关系，或说明由哪个功能检验项 / smoke test 间接覆盖。

> 注：本 L3 未覆盖"基准 topic 乱序"这一子场景。当前实现中，如果基准 topic 存在但时间戳无序（OUT_OF_ORDER），代码会标记为 `baseline_topic_out_of_order` 并加入 hard_fail。但该场景未包含在测试用例中。建议在后续 L3（如 `service_s3_006`）或补测中补充这一边缘用例。
...
```text
用户指定路径：DOCS/03_工程/阶段二：数据清洗/03_tasks/task/active/service-s3-g2/service_s3_005_实现MCAP_A输入盘点与校验服务.md
实际读取路径：DOCS/03_工程/阶段二：数据清洗/03_tasks/task/active/service-s3-g2/service_s3_005_实现MCAP_A输入盘点与校验服务.md
文件名编号：service_s3_005
正文 L3 编号：service_s3_005
dispatch.task_id：service_s3_005
校验结论：通过
```

### 调度与开工自检

- 调度元数据校验：`task_id=service_s3_005`、`task_file` 匹配、`group=service-s3-g2`、`branch=service-s3`、`dispatch_status=ready`、`depends_on=[service_s3_004]`（已归档于 `completed/04-service-s3/service-s3-g2/`），均允许执行；当前分支为 `service-s3`。
- 上游依赖：`depends_on=[service_s3_004]` 已完成并归档。
- 相关 L3 历史记录：
  ...
  - 已找到并读取 `DOCS/03_工程/阶段二：数据清洗/03_tasks/task/completed/03-service-s2/service-s2-g5/service_s2_019_补充MCAP_A契约与场景三兼容验收.md`
- 开工自检：`bash scripts/init_data_clean_dev.sh` 通过，输出 `Data clean dev environment OK`。

### 本次修改文件

- `src/data_clean/repo/mcap_topic_catalog.py`（新增）：Repo 层只读 MCAP topic 元数据提取器。提供 `TopicFact` dataclass 和 `extract_topic_facts()` 函数，遍历 MCAP 文件收集每个 topic 的名称、消息类型、样本数和时间戳列表。零业务规则。
- `src/data_clean/service/mcap_a_input_validator.py`（新增）：Service 层 MCAP_A 输入盘点与校验服务。提供 `validate_mcap_a_input()` 函数，编排：
  - MCAP_A 文件存在性检查
  - McapAWriteSummary 解析与 consistency 校验（status、path、policy）
  - topic catalog 构建
  - field entries 构建（基于 TargetFieldMapping 交叉引用）
  - 基准 topic（左右图像）存在性与交集计算
  - 非基准字段 warning 收集
  - 未映射 topic 盘点
  - SourceTopicCatalog 和 McapAInputValidationSummary 生成
- `src/data_clean/tests/service/test_mcap_a_input_validator.py`（新增）：15 个测试用例覆盖合法输入、summary hard fail（6 种）、基准 topic 缺失与无交集、非基准字段 warning、未映射 topic。
- `src/data_clean/data_clean_architecture.md`：在 repo 和 service 目录表中新增对应条目。

### TDD 过程

| 阶段 | 行为 | 结果 |
|------|------|------|
| Red | 编写 `test_mcap_a_input_validator.py`（首批 2 个测试），运行 `python3 -m pytest` | 2 failed（模块不存在），符合预期 |
| Green | 实现 `repo/mcap_topic_catalog.py` 和 `service/mcap_a_input_validator.py` | 2 passed |
| Red | 追加 6 个 summary hard fail 测试 | 8 passed ✅ |
| Red | 追加 3 个 baseline topic hard fail 测试 | 11 passed ✅ |
| Red | 追加 2 个 optional field warning + 2 个 unmapped topic 测试 | 15 passed ✅ |
| Refactor | 更新 `data_clean_architecture.md`，代码风格对齐现有模式 | 无测试回归 |
...
cd /home/hit/ROS && PYTHONPATH=src/data_clean python3 -m pytest \
  src/data_clean/tests/service/test_mcap_a_input_validator.py -q
# → 15 passed

# 2. 全量测试（排除已知 runtime 模块缺失问题和 pre-existing contract failure）
cd /home/hit/ROS && PYTHONPATH=src/data_clean python3 -m pytest \
  src/data_clean/tests -q --tb=line --ignore=src/data_clean/tests/runtime
# → 375 passed, 9 skipped, 2 failed（2 个失败为 pre-existing contract test 问题，非本 L3 导致）

# 3. 内联验证
cd /home/hit/ROS && PYTHONPATH=... python3 -c "..."
# → All inline verifications PASSED
```

### 成功标准处理

- [x] 合法 MCAP_A + completed summary 可输出 `SourceTopicCatalog` 和 `McapAInputValidationSummary(status=consumable)`：test_legal_input_produces_consumable 和 test_legal_input_field_entries 覆盖。
- [x] MCAP_A 缺失、summary 缺失 / 不可读 / 不一致会输出 hard fail：6 个 TestSummaryHardFail 测试覆盖缺失 MCAP_A、缺失 summary、不可读 summary、status 不一致、路径不匹配、policy 不匹配。
- [x] 左右图像基准 topic 缺失、无共同有效时间范围会阻塞：test_missing_left_image、test_missing_right_image、test_baseline_no_intersection 覆盖。基准 topic 乱序场景输出 hard fail 但未包含在测试用例中（见备注）。
- [x] 非基准字段问题只进入 warning 或 unavailable，不阻塞主链路：test_missing_optional_field_produces_warning_not_hard_fail、test_tactile_field_type_mismatch_warns_only 覆盖。
- [x] 未映射 topic 被记录为只读盘点，不作为错误：test_unmapped_topics_recorded_in_catalog、test_unmapped_topics_do_not_cause_hard_fail 覆盖。
- [x] 已说明本 L3 与 `./start_data_clean.sh --dev` 开发者验收入口的关系。

### 开发者验收入口关系

- 本 L3 不修改 `./start_data_clean.sh --dev`、开发者菜单、脚本调用或运行日志。
- 本 L3 的服务函数 `validate_mcap_a_input()` 直接支撑 `scene3_mcap_a_input_check` 功能检验项（在 `./start_data_clean.sh --dev` 下选择场景三）。
- 本 L3 的自动化验收只证明 MCAP_A 输入盘点服务局部实现正确；场景最终验收需要场景三全部 L3（`service_s3_001` ~ `service_s3_006`）完成后运行完整 smoke test。

### 当前没做

- 未实现基准 topic 乱序的测试用例（代码逻辑已实现 `baseline_topic_out_of_order` hard fail，但无 pytest 覆盖）。
- 未接入开发者菜单或 run 目录写出（由 service_s3_006 负责）。
...
- 测试需要 `PYTHONPATH` 包含 `src/data_clean` 和 mcap 第三方库路径，由 `start_data_clean.sh` 自动处理。

### 归档说明

- 本任务完成后移动到 `DOCS/03_工程/阶段二：数据清洗/03_tasks/task/completed/04-service-s3/service-s3-g2/`。
- 移动后如果原 active 功能组目录为空，则删除该空目录。
