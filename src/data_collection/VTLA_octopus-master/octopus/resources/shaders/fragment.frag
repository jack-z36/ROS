...
void main()
{
    vec3 albedo     = vec3(factor.base_color);

    vec3 light_position = vec3(0.0f, 4.0f, 1.5f);
    vec3 light_color = vec3(10.0f, 10.0f, 10.0f);
    vec3 camera_position = vec3(0.0f, 0.0f, 8.0f);

    vec3 N = normalize(Normal);
    vec3 V = normalize(camera_position - FragPosition);

    vec3 F0 = vec3(0.04);
    F0 = mix(F0, albedo, vec3(factor.metallic));
    ...
    for (int i = 0; i < 4; ++i)
    {
        ...
        // Cook-Torrance BRDF
        float NDF = DistributionGGX(N, H, factor.roughness);
        float G   = GeometrySmith(N, V, L, factor.roughness);
        vec3 F    = fresnelSchlick(max(dot(H, V), 0.0), F0);

        vec3 numerator    = NDF * G * F;
        float denominator =  4.0f * max(dot(N, V), 0.0) * max(dot(N, L), 0.0) + 0.0001;// + 0.0001 to prevent divide by zero
