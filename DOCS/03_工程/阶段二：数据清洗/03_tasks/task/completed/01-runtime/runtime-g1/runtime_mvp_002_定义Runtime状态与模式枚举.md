...
2. `DOCS/03_工程/阶段二：数据清洗/01_runtime_mvp/L2数据定义/RunStatus.md`
3. `DOCS/03_工程/阶段二：数据清洗/01_runtime_mvp/L2数据定义/RunMode.md`
4. `DOCS/03_工程/阶段二：数据清洗/01_runtime_mvp/L2数据定义/ServiceMode.md`
5. `DOCS/03_工程/阶段二：数据清洗/01_runtime_mvp/L2数据定义/SceneName.md`
...
```bash
python3 -m pytest src/data_clean/tests/runtime/test_runtime_context_enums.py
```
...
- [x] [[RunStatus]]、[[RunMode]]、[[ServiceMode]]、[[SceneName]] 已在 Types/Schemas 层定义。
- [x] 四个枚举能被 import。
- [x] 合法取值与原子数据定义一致。
- [x] 未引入未经确认的状态或场景。
...
1. `src/data_clean/schemas/runtime_enums.py` - 新增，定义四个 Runtime 枚举。
2. `src/data_clean/schemas/__init__.py` - 更新，导出四个枚举。
3. `src/data_clean/tests/runtime/test_runtime_context_enums.py` - 新增，11 个测试用例。
4. `src/data_clean/data_clean_architecture.md` - 更新，添加 `runtime_enums.py` 到架构表。

### 新增函数/测试

- `RunStatus` 枚举：`created`, `running`, `succeeded`, `failed`, `cancelled`
- `RunMode` 枚举：`dev_single_scene`, `dev_full_pipeline`, `prod_single_scene`, `prod_full_pipeline`
- `ServiceMode` 枚举：`fake`, `real`
- `SceneName` 枚举：`scene1`, `scene2`, `scene3`, `scene4`, `scene5`
- 测试类：`TestRunStatus`, `TestRunMode`, `TestServiceMode`, `TestSceneName`
- 11 个测试方法覆盖取值完整性、可序列化性、非法值排除（`partial_success`, `scene6`）和 pipeline 顺序。

### 验收命令

```bash
PYTHONPATH=/home/hit/ROS/src python3 -m pytest src/data_clean/tests/runtime/test_runtime_context_enums.py -v
python3 -m compileall src/data_clean/schemas
```
...
- 未定义 `RunContext` 字段对象。
- 未定义 `SceneResult`、`PipelineResult`、`RuntimeStepRecord` 或 `RuntimeErrorRef`。
- 未实现状态迁移逻辑。
- 未实现调度、日志、manifest 或错误摘要。
- 未修改 `runtime/`、`service/`、`repo/`、`config/`、`ui/` 中的实现。

### 下一步建议

- 执行 `runtime_mvp_003` 或后续 L3 任务，定义 `RunContext` 数据对象。
- 根据需要实现状态迁移和调度逻辑。
