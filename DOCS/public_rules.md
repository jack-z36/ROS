...
- 输出程序与文件：`/home/hit/ROS/DOCS/场景一/输出程序与文件.md`
...
- 输出程序与文件：`/home/hit/ROS/DOCS/场景二/输出程序与文件.md`
...
- 输出程序与文件：`/home/hit/ROS/DOCS/场景三/输出程序与文件.md`
...
- 输出程序与文件：`/home/hit/ROS/DOCS/场景四/输出程序与文件.md`
...
- 输出程序与文件：`/home/hit/ROS/DOCS/场景五/输出程序与文件.md`
...
- 输出程序与文件：`/home/hit/ROS/DOCS/场景六/输出程序与文件.md`
...
记录该场景已经形成或依赖的所有程序、脚本和配置文件。

应记录：

- 输出清单表格：类型、文件地址、功能简介。
- 场景目标输出表格：该场景最终产出的 topic、目录或可观测结果。
- 使用边界：本场景的产出范围和不负责的事项。

**架构文档要求：**

除输出清单中的简表外，每个程序、脚本或独立模块**必须**配备一份独立的架构说明文档（`*_architecture.md` 或 `*_架构说明.md`），放在该程序所属的代码包目录下。

架构文档必须包含以下内容，使毫无上下文记忆的后续 Agent 能够快速理解该程序的设计与运行方式：

1. **概述**：该程序的用途、所属场景、解决什么问题。
2. **目录结构**：源码文件清单及每个文件的职责。
3. **数据流**：输入来源、处理逻辑、输出目标（推荐用文字或 Mermaid 图描述）。
...
- 每个新场景至少包含目标描述、背景信息、执行约束、执行记录、当前进度、输出程序与文件六类文档。
