# L3 验收卡片：deploy_009 observation_collector 改造为 TCP+width 装配
...
|---|---|
| L3 编号 | `deploy_009` |
| L3 文件 | `DOCS/03_工程/阶段四：模型部署/03_tasks/task/active/l2-03-assembly/deploy_009_observation_collector改造.md` |
| 所属 L2 | `l2-03-assembly` |
| 验收卡片 | `DOCS/03_工程/阶段四：模型部署/03_tasks/cards/l2-03-assembly/deploy_009_验收卡片.md` |
| 对应 L2 运行场景 | `[S1, S2, S3]` |
| 验收模式 | `static-review` |
| 辅助模式 | `[downstream-l2]` |
| 本地验收是否必须 | `false` |
...
确认执行 agent 是否完成 `observation_collector 改造为 TCP+width 装配` 的唯一目标，并在 Ubuntu 22.04 无外联硬件条件下给出可验证结论。

## 验收模式说明

主模式：`static-review`。辅助模式：`[downstream-l2]`。

验收重点：S1/S2/S3 由 deploy_012 和 L2 Gate dry-run 聚合验收。
...
```text
DOCS/03_工程/阶段四：模型部署/05_acceptance/l2-03-assembly/logs/deploy_009_acceptance_round_<n>.md
```
