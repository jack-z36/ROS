# L3 验收卡片：deploy_004 Types 层单测覆盖
...
|---|---|
| L3 编号 | `deploy_004` |
| L3 文件 | `DOCS/03_工程/阶段四：模型部署/03_tasks/task/active/l2-01-types/deploy_004_types层单测.md` |
| 所属 L2 | `l2-01-types` |
| 验收卡片 | `DOCS/03_工程/阶段四：模型部署/03_tasks/cards/l2-01-types/deploy_004_验收卡片.md` |
| 对应 L2 运行场景 | `[S1, S2, S3]` |
| 验收模式 | `direct-local` |
| 辅助模式 | `[]` |
| 本地验收是否必须 | `true` |
...
确认执行 agent 是否完成 `Types 层单测覆盖` 的唯一目标，并在 Ubuntu 22.04 无外联硬件条件下给出可验证结论。

## 验收模式说明

主模式：`direct-local`。辅助模式：`[]`。

验收重点：S1/S2/S3 Types 层测试。
...
```text
DOCS/03_工程/阶段四：模型部署/05_acceptance/l2-01-types/logs/deploy_004_acceptance_round_<n>.md
```
