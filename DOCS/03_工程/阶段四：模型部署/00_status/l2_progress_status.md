...
| l2-03-assembly | `model_deploy-l2-03-assembly` | 不通过（BLOCKED_ENV） | 不允许 L2-04 | 4 个 required L3 均已 committed-local 并推送；deploy_009/010/011 `PASS_LOCAL`，deploy_012 `BLOCKED_ENV`（torch 缺失，5/6 case 跳过）；待 torch 环境补齐重跑 012 全通过即可放行 |
