...
- [ ] `success` 行为能返回成功 [[FakeServiceResult]]。
- [ ] `controlled_failure` 行为能返回带 `fake_service_controlled_failure` 的失败结果。
- [ ] 非 fake mode、缺目标场景、输出路径逃逸均失败清楚。
- [ ] 不读取或写入真实 MCAP / canonical dataset / exports。
...
- `DOCS/阶段二：数据清洗/执行记录/<MMDDHH_runtime_mvp_021_fake_service_result_generator>.md`
