...
当前任务文件路径：`DOCS/03_工程/阶段二：数据清洗/03_tasks/task/active/service-s3-g5/service_s3_015_定义AlignmentReport阶段字段与统计类型.md`  
任务类别：数据定义类  
...
dispatch:
  ...
  task_file: DOCS/03_工程/阶段二：数据清洗/03_tasks/task/active/service-s3-g5/service_s3_015_定义AlignmentReport阶段字段与统计类型.md
  group: service-s3-g5
  branch: service-s3
  wave: 1
...
5. `DOCS/03_工程/阶段二：数据清洗/02_service/场景三/L2数据定义/FieldAlignmentResult.md`

### 必读相关微元任务记录

1. `DOCS/03_工程/阶段二：数据清洗/03_tasks/task/completed/04-service-s3/service-s3-g1/service_s3_002_定义step时间轴与对齐索引类型.md`
2. `DOCS/03_工程/阶段二：数据清洗/03_tasks/task/completed/04-service-s3/service-s3-g4/service_s3_010_定义FieldAlignmentResult类型与策略契约.md`
...
- [x] 已定义 `AlignmentReport` 或等价代码类型。
- [x] 类型能表达 draft 阶段输出路径为空、final 阶段输出路径补齐。
- [x] 已覆盖 `completed` / `degraded` / `failed` 状态和 `degradation_summary`。
- [x] 未实现 AlignmentIndex 规范化、report 统计服务或任何写出动作。
...
| 项目 | 结果 |
|---|---|
| 用户指定路径 | `DOCS/03_工程/阶段二：数据清洗/03_tasks/task/active/service-s3-g5/service_s3_015_定义AlignmentReport阶段字段与统计类型.md` |
| 实际读取路径 | 匹配 |
| 文件名编号 | `service_s3_015` |
| 正文 L3 编号 | `service_s3_015` |
| task_id | service_s3_015 |
| group | service-s3-g5 |
| branch | service-s3 (当前分支确认) |
| dispatch_status | ready |
| depends_on | [service_s3_002, service_s3_010] → 均已归档 |
| 分支校验 | service-s3 ✓ |
...
| 切片 | 测试内容 | 状态 |
|---|---|---|
| VS1 RED→GREEN | `AlignmentReportStatus` enum (completed/degraded/failed) | ✓ |
| VS2 RED→GREEN | `AlignmentFieldStats` dataclass (per-field counts, avg_dt_ms, coverage_ratio) | ✓ |
| VS3 RED→GREEN | `AlignmentDegradationSummary` dataclass (missing, timeout, fallback, unavailable) | ✓ |
| VS4 RED→GREEN | `AlignmentReport` typed fields + `__post_init__` validation | ✓ |
| REFACTOR | Full test sweep: 401 passed, 1 pre-existing fail, 9 skipped | ✓ |

### 修改文件

| 文件 | 改动 |
|---|---|
| `src/data_clean/schemas/aligned_mcap_report.py` | +AlignmentReportStatus enum, +AlignmentFieldStats, +AlignmentDegradationSummary, 更新 AlignmentReport 使用 typed fields + __post_init__ validation |
| `src/data_clean/schemas/__init__.py` | 导出 AlignmentReportStatus, AlignmentFieldStats, AlignmentDegradationSummary |
| `src/data_clean/tests/service/test_aligned_mcap_report_schemas.py` | 新增 4 个测试类 (TestAlignmentReportStatus, TestAlignmentFieldStats, TestAlignmentDegradationSummary, TestAlignmentReportValidation, TestNewTypesImportable) |
...
# 全部 schema 测试
python3 -m pytest src/data_clean/tests/service/test_aligned_mcap_report_schemas.py -v -q
# → 34 passed, 1 failed (pre-existing data_clean import issue, unrelated)

# 导入验证
python3 - <<'PY'
import sys; sys.path.insert(0, 'src/data_clean')
from schemas import AlignmentReport, AlignmentReportStatus, AlignmentFieldStats, AlignmentDegradationSummary
assert AlignmentReport is not None
assert AlignmentReportStatus.completed.value == 'completed'
assert AlignmentFieldStats().total_count == 0
assert AlignmentDegradationSummary().missing_time_count == 0
print('All type imports verified')
PY
```

### 与开发者验收入口的关系

本 L3 定义了 AlignmentReport 类型体系，是 `scene3_alignment_report_check` 功能检验项的直接数据契约基础。实际报告生成和开发者入口由后续 L3 (service_s3_016, service_s3_017, service_s3_018) 实现。建议用户在完成 service-s3-g5 全部 L3 后运行 `./start_data_clean.sh --dev` 选择场景三 → `scene3_alignment_report_check` 做最终人工验收。

### 遗留风险

- `data_clean.schemas` 的顶层包名导入 (`from data_clean.schemas import ...`) 在 pytest 环境下因 sys.path 配置无法工作，使用 `from schemas import ...` 替代。这是测试环境的已知限制，不影响生产代码。
- 新的 `AlignmentFieldStats` 和 `AlignmentDegradationSummary` 类型目前由下游统计服务消费，尚未在生成侧使用；service_s3_017 将首次使用。
