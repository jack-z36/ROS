...
任务类别：流程编排类  
来源 L2 文件：`DOCS/03_工程/阶段二：数据清洗/01_runtime_mvp/L2能力模块/07_Fake Service模块.md`
...
- DOCS/03_工程/阶段二：数据清洗/03_tasks/task/active/runtime-g6/runtime_mvp_013_service_dispatch_types.md
...
1. `DOCS/03_工程/阶段二：数据清洗/01_runtime_mvp/L2能力模块/07_Fake Service模块.md`
2. `DOCS/03_工程/阶段二：数据清洗/01_runtime_mvp/L2能力模块/06_场景注册与Service调度模块.md`
3. `DOCS/03_工程/阶段二：数据清洗/01_runtime_mvp/L2数据定义/FakeServiceResult.md`
4. `DOCS/03_工程/阶段二：数据清洗/01_runtime_mvp/L2数据定义/ServiceBinding.md`
5. `DOCS/03_工程/阶段二：数据清洗/01_runtime_mvp/L2数据定义/SceneDispatchPlan.md`

### 必读相关微元任务记录

1. `DOCS/03_工程/阶段二：数据清洗/03_tasks/task/active/runtime-g6/runtime_mvp_013_service_dispatch_types.md`
2. `DOCS/03_工程/阶段二：数据清洗/03_tasks/task/active/runtime-g6/runtime_mvp_014_service_registry.md`
3. `DOCS/03_工程/阶段二：数据清洗/03_tasks/task/active/runtime-g6/runtime_mvp_015_single_scene_dispatcher.md`
4. `DOCS/03_工程/阶段二：数据清洗/03_tasks/task/active/runtime-g7/runtime_mvp_020_fake_service_types.md`
5. `DOCS/03_工程/阶段二：数据清洗/03_tasks/task/active/runtime-g7/runtime_mvp_021_fake_service_result_generator.md`
...
- [x] fake service 可通过 [[ServiceBinding]] 或等价 adapter 被调用。
- [x] 成功 [[FakeServiceResult]] 能转换为成功 [[SceneResult]]。
- [x] 失败 [[FakeServiceResult]] 能转换为携带 [[RuntimeErrorRef]] 的失败 [[SceneResult]]。
- [x] 本任务没有实现完整单场景或全流程调度器。
...
- `DOCS/03_工程/阶段二：数据清洗/03_tasks/task/completed/runtime-g7/runtime_mvp_020_fake_service_types.md`
- `DOCS/03_工程/阶段二：数据清洗/03_tasks/task/completed/runtime-g7/runtime_mvp_021_fake_service_result_generator.md`
- `src/data_clean/schemas/runtime_dispatch_types.py` - 了解 ServiceBinding、SceneResult 定义

### 修改的文件

- `src/data_clean/runtime/fake_service_dispatch_adapter.py` - 新增，包含 FakeServiceDispatchAdapter
- `src/data_clean/runtime/__init__.py` - 新增导出
- `src/data_clean/tests/runtime/test_fake_service_dispatch_adapter.py` - 新增测试

### 新增或修改的函数 / 测试

- `FakeServiceDispatchAdapter` 类：
  - `invoke(plan) -> SceneResult`：调用 fake service 并转换为 SceneResult
  - `__call__` 方法：使adapter可调用
- `dispatch_fake_service(plan)` 便捷函数
- 测试覆盖 7 个用例全部通过

### TDD red / green / refactor 执行

- RED：测试文件创建，模块不存在导致 ImportError
- GREEN：实现 fake_service_dispatch_adapter.py，7 个测试通过
- REFACTOR：无需重构

### 验收命令

```bash
PYTHONPATH=src python3 -m pytest src/data_clean/tests/runtime/test_fake_service_dispatch_adapter.py -v
```
...
- [x] fake service 可通过 adapter 被调用
- [x] 成功 FakeServiceResult 转换为成功 SceneResult
- [x] 失败 FakeServiceResult 转换为带 RuntimeErrorRef 的失败 SceneResult
- [x] 未实现完整调度器
- [x] 验收命令通过
...
- 不实现完整单场景或全流程调度器
- 不写入日志、manifest、error summary

### 下一步建议

- 功能7 Fake Service 模块已完成 L3-020/021/022，可进入 Runtime MVP 整体验证
