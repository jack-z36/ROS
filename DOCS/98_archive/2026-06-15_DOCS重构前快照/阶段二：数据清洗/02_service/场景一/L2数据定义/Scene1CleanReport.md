...
| `pose_topics` | list | raw pose、arm-base TCP pose 的消息数量和 payload 语义；旧 `tcp_common` / `camera_common` 标记为历史字段统计 |
| `gripper_topics` | list | 每个 gripper topic 的图像帧数、输出数、缺失帧和插值帧 |
| `raw_pose_retention` | string | raw pose 保留或备份策略 |
| `common_anchor` | string | [[FrameAlignmentConfig]] 使用的 common anchor（已废弃，保留历史兼容） |
| `tcp_extrinsic_source` | string | [[CameraFromTcpExtrinsic]] 来源 |
...
- `pose_topics` 必须说明 raw pose 是否保留、[[ArmBaseTcpPose]] 的输出数量；旧 `tcp_common` / `camera_common` 仅做历史字段统计。
